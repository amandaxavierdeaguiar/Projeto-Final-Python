...
def apply_class_bindings(window: tkinter.Widget):
    ...
    for className in ["TEntry", "TSpinbox", "TCombobox", "Text"]:
        ...
        for sequence in ["<Control-a>", "<Control-A>"]:
            window.bind_class(
                className=className, sequence=sequence, func=on_select_all
            )
    ...
    window.bind_class("TButton", "<Key-Return>", button_default_binding, add="+")
...
if __name__ == "__main__":
    root = BaseWindow(themename="superhero", alpha=0.5, size=(1000, 1000))
    # root.withdraw()
