...
class InsertUser:
    ...
    def __init__(self):
        ...
        self.create_form_user("Password Confirmation:", self.password_confirm)
        self.create_form_user("Tipo de Acesso:", self.typeAcess.get())"""
        
        self.create_form_user("Nome:", self.name)
        self.create_form_user("Login:", self.login)
        self.create_form_user("Password:", self.password)
        self.create_form_user("Password Confirmation:", self.password_confirm)
        self.create_form_user("Tipo de Acesso:", self.typeAcess)
    ...
    def frame_photo_user(cls):
        ...
        cls.button1 = Label(
            prod_frame_description, width=250, height=250, image=cls.img_user, text=""
    ...
    def create_form_user(cls, label, variable): 
        ...
        if label == "Password:":
            ent = Entry(
                master=container, textvariable=variable, show="*", bg="white", relief="solid", borderwidth=0.5)
        else:
            ent = Entry(
                master=container, textvariable=variable, bg="white", relief="solid", borderwidth=0.5)
    ...
    def create_buttonbox(cls):
        """Caixa Botoes App"""
        container = Frame(cls.root)
        container.pack(fill=X, expand=YES, pady=(15, 10))
        
        check_btn = Button(
            master=container,
            text="Check Password",
            command=cls.are_passwords_same,
            bootstyle="INFO",
            width=12,
        )
    ...
    @classmethod
    def are_passwords_same(cls):
        
        return cls.password.get() == cls.password_confirm.get()
